...
let storage = multer.diskStorage({
    destination: function (req, file, cb) {
        ...
        let dirname =  __dirname + '/../UPLOAD/' +(new Date().getFullYear())+"/"+(new Date().getMonth()+1)+'/'+(new Date().getDate())+'/';
...
router.post('/', upload.single('file'), (req, res) => {
    ...
    let dirname = 'UPLOAD/'+(new Date().getFullYear())+"/"+(new Date().getMonth()+1)+'/'+(new Date().getDate())+'/';
